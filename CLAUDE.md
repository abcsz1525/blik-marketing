...
- [x] ServicesGrid: 6 направлений БЛИК
...
### 5.2 Услуги (`/services`)
- [x] ServicesHero
- [x] ServicesList: карточки — Исследования и стратегии, Позиционирование, Дизайн и креатив, SMM & ORM, PR и медиа, Антикризисное управление
- [ ] CTA в конце
...
- [ ] **AgreementBody h1 missing `lang={locale}` + `useLocale` hook.** PrivacyBody и NotFoundBody следуют паттерну `useLocale()` + `lang={locale}` + `hyphens-auto` (через глобальное CSS-правило на `.display-xl/lg/md`). AgreementBody пропущен — accidentally correct на RU через hardcoded `<html lang="ru">`, но инконсистентен. Fix: добавить hook + атрибут, 3 строки. **Priority: LOW** (hyphenation работает через parent lang).

- [ ] **`industryId: string` in `CaseStudy`** — должен быть union type `IndustryId`, derived from `typeof INDUSTRIES[number]['id']`. Текущий `string` не предотвращает typo'ы при добавлении кейсов. В Sprint 3D мы обнаружили baseline проблему когда Arlight/Столплит были в `real-estate` (не-существующая по факту ошибка, а technically valid `string`). Fix: добавить `export type IndustryId = typeof INDUSTRIES[number]['id']` в `industries.ts`, поменять тип в `cases.ts`. **Priority: LOW**.
